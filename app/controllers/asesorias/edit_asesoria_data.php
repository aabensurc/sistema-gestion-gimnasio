...
// Este archivo se incluye en asesorias/edit.php para cargar los datos de una asesoría específica.
...
if (empty($id_asesoria_get)) {
    session_start();
    $_SESSION['mensaje'] = "Error: ID de asesoría no proporcionado para edición.";
...
try {
    ...
    $sql_asesoria = "SELECT
                  ...
                  WHERE a.id_asesoria = :id_asesoria_get AND c.id_gimnasio = '$_SESSION[id_gimnasio_sesion]'"; // Asegurar que la asesoría pertenece al gimnasio de la sesión
    ...
    if (!$asesoria_data) {
        session_start();
        $_SESSION['mensaje'] = "Error: Asesoría no encontrada para edición.";
        $_SESSION['icono'] = "error";
        header('Location: ' . $URL . '/asesorias/');
        exit();
    }
    ...
    session_start();
    $_SESSION['mensaje'] = "Error de base de datos al cargar la asesoría para edición: " . $e->getMessage();
    $_SESSION['icono'] = "error";
    header('Location: ' . $URL . '/asesorias/');
    exit();
}

// La variable $asesoria_data ahora está disponible para la interfaz edit.php
